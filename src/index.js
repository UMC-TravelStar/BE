...
app.delete("/background", deleteBack); // 일지 작성 화면 배경화면 삭제
 ...
 *     tags: [mainpage]
 *     summary: 메인 페이지에서 검색
 *     description: "사용자가 입력한 검색어로 포스트를 검색합니다."
 *     parameters:
 *       - in: query
 *         name: term
 *         required: true
 ...
 *     tags: [schedule]
 *     summary: 일정 추가
 *     description: 사용자가 새로운 일정을 추가합니다.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *                 type: string
 *                 format: date-time
 *                 description: 일정의 날짜 및 시간 (ISO 8601 형식)
 *     responses:
 *       201:
...
/**
 * @swagger
 * /prod/schedule/{date}/{schedule_id}:
 *   get:
 *     tags: [schedule]
 *     summary: 특정 일정 조회
 ...
 *         in: path
 *         required: true
 *         description: 일정이 있는 날짜 (ISO 8601 형식)
 *         schema:
 *           type: string
 *       - name: schedule_id
 *         in: path
